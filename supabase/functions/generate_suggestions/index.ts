...
/**
 * Main function that generates holistic health suggestions
 * Extracted for better structure and potential reuse
 */
async function generateHolisticSuggestions(user_id: string) {
  const supabaseUrl = Deno.env.get("SUPABASE_URL") ?? "";
  const supabaseServiceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
  const supabase = createClient(supabaseUrl, supabaseServiceKey);

  // 1. Fetch TARGET USER Context (Profile + All Records)
  ...
  // 2. Fetch FAMILY Context (Links + Profiles + ALL their Records)
  ...
  // 3. Construct the data packet for Claude
  const dataPacket = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Perform the analysis (this may take 10-20 seconds)
    const suggestionsCount = await generateHolisticSuggestions(user_id);
